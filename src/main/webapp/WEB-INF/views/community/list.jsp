			...
			<div class="panel panel-default">
				<div class="panel panel-heading">
					...
					<div class="hstack gap-3">
					    <div class=""><h3 class="mt-5 mb-3">커뮤니티</h3></div>
					    <div class="ms-auto mt-5 mb-3"><button type="button" class="btn btn-secondary mt-2" onclick="location.href='/community/register'">글쓰기</button></div>
					</div>    
				</div>
					...
					<table class="table table-hover table-sm">
						...
						<c:forEach var="community" items="${list}">
		                 	<tr>
		                    	...
		                    	<td><a class='move' href='<c:out value="${community.bno}"/>' style='text-overflow:ellipsis;overflow:hidden; word-break:break-all'>
		                    		<c:out value = "${community.title}"/><b>(<c:out value="${community.replyCnt}"/>)</b></a>
		                    	</td>
		                    	<td class='<c:out value="${community.writer}"/>'></td>
		                    	<td><c:out value="${community.writer}"/></td>
		                    	<td><fmt:formatDate pattern="yyyy-MM-dd" value="${community.regDate}" /></td>
		                  	</tr>
		               </c:forEach>
					</table>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('.page-item a').on("click",
			...
			actionForm.attr("action", "/community/list");
			actionForm.submit();
		});
	});
</script>
...
<script type="text/javascript">
	...
	<c:forEach items="${list}" var="community">
		list.push('<c:out value="${community.writer}"/>');
	</c:forEach>
