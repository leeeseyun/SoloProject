			...
			<div class="panel panel-default">
				<div class="panel panel-heading">
					...
					<div class="hstack gap-3">
					    <div class=""><h3 class="mt-5 mb-3">이벤트</h3></div>
					    <div class="ms-auto mt-5 mb-3"><button type="button" class="btn btn-secondary mt-2" onclick="location.href='/event/register'">글쓰기</button></div>
					</div>    
				</div>
					...
					<table class="table table-hover table-sm">
						...
						<c:forEach var="event" items="${list}">
		                 	<tr>
		                    	...
		                    	<td><a class='move' href='<c:out value="${event.bno}"/>' style='text-overflow:ellipsis;overflow:hidden; word-break:break-all'>
		                    		<c:out value = "${event.title}"/><b>(<c:out value="${event.replyCnt}"/>)</b></a>
		                    	</td>
		                    	<td class='<c:out value="${event.writer}"/>'></td>
...
<script type="text/javascript">
	...
	<c:forEach items="${list}" var="event">
		list.push('<c:out value="${event.writer}"/>');
	</c:forEach>
		...
		memberProfileService.getMemberProfile(i,
			function(data){
				...
				if(data.userId == loginId && data.uuid == 'default'){       //로그인유저 사진이 없을때
					...
					'<li><a class="dropdown-item" href="/event/list?type=W&keyword='+data.userId+'&pageNum=1&amount=10">내 게시글 보기</a></li>'+
                ...
                '<li><a class="dropdown-item" href="#">좋아요한 게시물</a></li></ul>';		//security
                    ...
                    html = '<a class="nav-link dropdown-toggle" data-bs-toggle="dropdown" href="#" role="button" aria-expanded="false"><div style="float:left;"><img src="/resources/img/default-profile.svg"></div></a>'+
						'<ul class="dropdown-menu">'+
						'<li><a class="dropdown-item"><strong>'+data.userId+'</strong></a></li><li><hr class="dropdown-divider"></li>'+
					'<li><a class="dropdown-item" href="/event/list?type=W&keyword='+data.userId+'&pageNum=1&amount=10">게시글 보기</a></li>'+
